...
class MetricsAggregator:
    ...
    def __init__(self, base_path: str = "data/metrics"):
        ...
        # Создаем директории
        for path in [self.hourly_path, self.daily_path, self.weekly_path, self.monthly_path]:
    ...
    async def start_aggregation(self, get_metrics_func: Callable[[], Awaitable[Dict[str, Any]]]) -> None:
    ...
    async def _perform_hourly_aggregation(self, get_metrics_func: Callable[[], Awaitable[Dict[str, Any]]]) -> None:
    ...
    def _create_hourly_summary(self, metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "monitoring_page_views": basic.get("counters", {}).get("monitoring_page_views", 0),
    ...
    async def _perform_daily_aggregation_if_needed(self) -> None:
        ...
        try:
            ...
            logger.info(f"Дневная агрегация выполнена: {day_key} ({len(hourly_data)} часов)")
            
        except Exception as e:
            logger.error(f"Ошибка дневной агрегации: {e}")
    
    async def _collect_hourly_data_for_day(self, target_date: datetime) -> List[Dict[str, Any]]:
    ...
    def _create_daily_summary(self, hourly_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for hour_data in hourly_data:
            ...
            summary["monitoring_page_views"] += hour_summary.get("monitoring_page_views", 0)
    ...
    def _create_hourly_breakdown(self, hourly_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    async def _cleanup_files_by_age(self, directory: Path, period_type: str, cutoff_date: datetime) -> None:
    ...
    def _parse_date_from_filename(self, filename: str, period_type: str) -> Optional[datetime]:
    ...
    async def _enforce_size_limits(self) -> None:
        ...
        try:
            ...
            logger.info(f"Размер данных {total_size_mb}MB превышает лимит {max_size_mb}MB")
            
            # Собираем все файлы с датами модификации
            all_files = []
            for directory in [self.hourly_path, self.daily_path, self.weekly_path, self.monthly_path]:
                for file in directory.glob("*.json"):
                    try:
                        stat = file.stat()
            ...
            if deleted_count > 0:
                logger.info(f"Удалено {deleted_count} файлов для соблюдения лимита размера")
    ...
    async def _save_aggregated_data(self, file_path: Path, data: Dict[str, Any]) -> None:
    ...
    def get_aggregation_status(self) -> Dict[str, Any]:
        ...
        status: Dict[str, Any] = {
            "running": self._aggregation_task is not None and not self._aggregation_task.done(),
            "total_size_bytes": self._calculate_total_size(),
            "max_size_bytes": self.max_total_size,
            "size_usage_percent": round((self._calculate_total_size() / self.max_total_size) * 100, 1),
            "directories": {}
        }
...
async def start_metrics_aggregation(get_metrics_func: Callable[[], Awaitable[Dict[str, Any]]]) -> None:
    """Запускает автоматическую агрегацию метрик."""
