...
def _try_fix(
    value: str | None, synonyms: dict[str, str], valid_set: frozenset[str]
...
def auto_fix_register(raw_reg: dict) -> tuple[dict, list[FieldError]]:
    ...
    if "format" in raw_reg:
        fixed, original = _try_fix(
            str(raw_reg["format"]), FORMAT_SYNONYMS, VALID_FORMATS
        )
    ...
    if "reg_type" in raw_reg:
        fixed, original = _try_fix(
            str(raw_reg["reg_type"]), REG_TYPE_SYNONYMS, VALID_REG_TYPES
        )
        if original is not None and fixed is not None:
    ...
    if "channel_type" in raw_reg:
        fixed, original = _try_fix(
            str(raw_reg["channel_type"]),
            CHANNEL_TYPE_SYNONYMS,
            VALID_CHANNEL_TYPES,
        )
        if original is not None and fixed is not None:
    ...
    if "access" in raw_reg:
        fixed, original = _try_fix(
            str(raw_reg["access"]),
            ACCESS_SYNONYMS,
            frozenset({"read", "write", "readwrite"}),
        )
        if original is not None and fixed is not None:
...
def validate_register(reg: Register) -> RegisterValidation:
    ...
    if (
        reg.readonly is not None
        and reg.read_only is not None
    ...
    if (
        reg.channel_type == "value"
        and not reg.enum
        and not reg.enum_entries
        and not reg.is_parameter
        and reg.reg_type not in BIT_REG_TYPES
    ):
...
def auto_fix_and_validate(
    ...
    for raw_reg in raw_registers:
        ...
        try:
            ...
            logger.warning(
                "Ошибка парсинга регистра после авто-фикса %s: %s", raw_reg, e
            )
            try:
                reg = Register(
