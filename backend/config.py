...
class Settings(BaseSettings):
    ...
    LLM_API_URL: str = ""  # если пусто — серверный LLM отключён, нужен свой в настройках
    ...
    # None = не отправлять параметр. gpt-5.x принимают только temperature=1 и отвергают 0;
    ...
    QUEUE_ACTIVATION_DELAY: float = 1.0    # Задержка перед стартом того, кто ждал в очереди (сек)
    ...
    # Пусто = кросс-доменные запросы запрещены. Интерфейс ходит через тот же origin, CORS ему не нужен.
