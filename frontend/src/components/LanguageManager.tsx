    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={onClose}>
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md mx-4 p-5" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-base font-semibold text-gray-900">{t('languages.title')}</h3>
              ...
              <div key={lang.code} className="flex items-center gap-2 bg-gray-50 rounded px-3 py-1.5">
                ...
                {llmAvailable && (
                  <button
                    ...
                    title={t('languages.translateAllTitle', { lang: lang.label })}
                  >
                    {translating ? '...' : t('languages.translateAll')}
                  </button>
                )}
                <button
                  onClick={() => removeLanguage(lang.code)}
                  className="text-red-400 hover:text-red-600 text-xs font-bold"
                  title={t('languages.deleteLang', { lang: lang.code })}
                >
                  x
                </button>
              </div>
        ...
        <div className="border-t pt-3">
          <div className="text-xs font-medium text-gray-500 mb-2">{t('languages.addTitle')}</div>
          <div className="flex gap-2 items-end">
            <div className="w-20">
              <label className="text-xs text-gray-500">{t('languages.code')}</label>
              <input
                type="text"
                value={newCode}
            ...
            <div className="flex-1">
              <label className="text-xs text-gray-500">{t('languages.label')}</label>
              <input
                type="text"
                value={newLabel}
