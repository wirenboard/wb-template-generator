...
- **i18n**: переключатель языка в шапке (циклический: RU → EN → KZ → IT)
- **i18n**: лёгкая система переводов без внешних зависимостей (`useT()` хук + `getT()` для non-React)
- **UX**: hero-блок при пустой таблице — 3 карточки: «Из документа (AI)», «Импорт шаблона», «Вручную / CSV»
...
- Шапка упрощена: только заголовок + версия + переключатель языка
