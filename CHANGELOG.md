...
- **`Makefile` — единый вход**: `make lint / test / build / deploy / rollback / smoke / conformance`. Одни и те же команды локально и в CI.
- **`ci/shell/`** — логика выката отдельно от YAML: `deploy.sh`, `rollback.sh`, `smoke.sh` (с ретраями), `guard_staleness.sh` (сторож устаревшей ветки), `last_good.sh` (break-glass-зеркало), `conformance.sh` (проверка репозитория на соответствие гейту).
...
- **shellcheck и `bash -n`** на скриптах выката (`make lint-shell`, входит в `make lint`).
