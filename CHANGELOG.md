...
### Добавлено
- **Выкат и откат по стандарту деплоя.** Образы собираются в CI и метятся git-SHA, сервер только скачивает готовый образ — сборки на сервере больше нет, поэтому любую прошлую версию можно вернуть за секунды. Конвейер: `push_other.yml` (проверки на ветках/PR), `push_master.yml` (main: сборка → выкат → smoke → журнал → тег), `rollback.yml` (откат одной кнопкой, цель берётся из журнала выкатов).
- **Конвейер собран из готовых действий**: `docker/build-push-action` (сборка и пуш с кэшем), `appleboy/scp-action` + `appleboy/ssh-action` (доставка и команды на сервере), `bobheadxi/deployments` (журнал), `dangoslen/changelog-enforcer` (CHANGELOG в PR). Шаги выката — в локальном composite-действии `.github/actions/deploy`, оно же переиспользуется откатом.
- **`Makefile` — единый вход**: `make lint / test / build / smoke / conformance`. Одни и те же команды локально и в CI.
- **`ci/shell/`** — только то, для чего готового действия нет: `smoke.sh` (живость + сверка `revision`), `guard_staleness.sh` (сторож устаревшей ветки), `conformance.sh` (проверка репозитория на соответствие гейту).
- **`docker-compose.deploy.yml`** — прод-конфигурация на образах по git-SHA из `ghcr.io`.
- **`DEPLOYING.md`** — операторская карточка: как выкатить, откатить и что делать при аварии, включая случай «GitHub недоступен».
- **`revision` в `/api/status`** — git-SHA работающего образа: видно, из какого коммита собрано то, что стоит в проде. Покрыто тестами (`tests/test_status_revision.py`).
- **Проверка `[Unreleased]` на пустоту в PR** — вернули из прежнего `ci.yml`: версия и git-тег берутся из первой секции `## [X.Y.Z]`, и записи, забытые в `[Unreleased]`, тихо оставляли релиз под старым номером.
- **`/healthz` у frontend** + HEALTHCHECK в образе: rolling-update видит, поднялся ли контейнер.
- **shellcheck и `bash -n`** на скриптах (`make lint-shell`, входит в `make lint`).

### Изменено
- **Единый порт `:9000` в backend-образе** (`CMD` и `HEALTHCHECK`): раньше они указывали на `:8000`, из-за чего образ не запускался сам по себе, а его HEALTHCHECK всегда падал вне compose.
- **`CMD` через `exec`** — uvicorn получает SIGTERM напрямую и завершается штатно, без десятисекундного добивания при каждом перезапуске.
- **nginx перечитывает адрес backend** (`resolver` + имя через переменную): при rolling-update старый закэшированный IP давал 502 на `/api` до перезапуска фронта.
- **Прод-секреты — в среде `production`**, шаг ssh пиннует действие по commit-SHA, отпечаток сервера берётся из `DEPLOY_KNOWN_HOSTS`.
- **Документация приведена в соответствие**: README (архитектура, структура, CI/CD, продакшен-деплой), `CLAUDE.md` (make-цели, правило «логика выката — в `ci/shell/`»), `docs/api.md` (`revision`).

### Удалено
- **`docker-compose.prod.yml`** — он собирал образ на боевом сервере. Такой выкат нечем откатить (образа нет в реестре), и это прямое нарушение гейта; прод-конфигурация теперь одна — `docker-compose.deploy.yml`.
- **`ci.yml`** — заменён тремя тонкими workflow (см. выше).
