...
### Добавлено
- **Система мониторинга**: встроенная страница мониторинга с LLM метриками, статистикой использования и анализом ошибок
- **Административная панель**: двухуровневый доступ (публичные метрики для всех, детальные для администраторов с ADMIN_TOKEN)
- **Классификация ошибок LLM**: автоматическая категоризация ошибок (авторизация, сеть, квоты, модель, токены, формат, парсинг, пустой ответ, API, прочие) с человекочитаемыми описаниями и рекомендациями
- **Метрики токенов и стоимости**: отслеживание использования prompt/completion токенов, приблизительный расчёт стоимости
- **Метрики качества**: статистика по извлечённым регистрам, авто-исправлениям, частоте ошибок и повторов
- **API эндпоинты мониторинга**: `/api/metrics` (публичные), `/api/admin/metrics` (детальные), `/api/metrics/page-view` (отслеживание)
- **UI компоненты**: страница мониторинга с авторизацией по токену, карточки метрик с компактным отображением больших чисел, детальные карточки ошибок с техническими деталями
- **Интернационализация мониторинга**: полная поддержка переводов страницы мониторинга на 4 языка (RU, EN, KZ, IT) включая названия метрик, категории ошибок, статусы, сообщения об ошибках и вкладки навигации
- **Система персистентности метрик**: JSON-файловое хранение с автосохранением каждые 5 минут, атомарными операциями записи, восстановлением состояния при старте
- **Многоуровневая агрегация данных**: автоматическая агрегация метрик (почасовая → дневная → недельная → месячная) с ротацией старых данных и контролем размера (100MB лимит)
- **API исторических метрик**: эндпоинты для доступа к почасовым/дневным данным `/api/metrics/history/hourly`, `/api/metrics/history/daily`, `/api/metrics/summary/latest`
- **Административные эндпоинты агрегации**: `/api/admin/metrics/history/*` и `/api/admin/metrics/aggregation` для детального мониторинга системы агрегации
- **Дашборд аналитики**: полностью локализованный UI с динамическими переводами, красивыми графиками и статистикой на 4 языках
- **Навигация аналитики**: кнопка "📈 Аналитика" в главной навигации с переводами, удобный переход и кнопка "Назад"

### Изменено
- **Страница мониторинга**: удалены элементы стоимости (оценочная и реальная) из UI для упрощения интерфейса
- **Навигация**: дашборд аналитики открывается в том же окне вместо новой вкладки для лучшего UX
- **Переводы**: исправлены динамические переводы в JavaScript коде, добавлены недостающие ключи для полной локализации
