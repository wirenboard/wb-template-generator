...
deployDockerService(
    ...
    // ВРЕМЕННО: файл выката пока живёт в этом репозитории и едет на хост при каждом
    // выкате. Целевое состояние — инфраструктурный репозиторий; переносится, когда
    // инфраструктура скажет, где место файла для этого хоста.
    stackFileInRepo: true,
