#!/usr/bin/env bash
# Smoke после выката: ключевые адреса отвечают 200 и сервис отдаёт выкаченный SHA.
# Ретраи обязательны — от результата зависит, откатывать ли, а одиночный сетевой
# чих не должен откатить исправную версию.
set -euo pipefail
