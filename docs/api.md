...
Файлов в запросе не больше `MAX_FILES` (10), размер каждого — не больше
`MAX_FILE_SIZE_MB` (2), тот же потолок размера у `/api/import-template`. Тело запроса
целиком режет nginx своим `client_max_body_size` (2m, задан статикой в образе и за
переменной не следует), поэтому фактический потолок — сумма файлов, и интерфейс
проверяет именно её. Настройка бэкенда при этом пофайловая — она второй рубеж для
клиентов мимо интерфейса, и при выставленных вровень числах отказ по ней приходит
только тем, кто пришёл мимо прокси.

Число файлов, размер и список расширений `/api/status` отдаёт полями `max_files`,
`max_file_size_mb` и `allowed_extensions` — интерфейс отсекает по ним набор до отправки,
