...
MAX_FILE_SIZE_MB=2     # Максимальный размер одного файла (МБ). Тело запроса режет nginx своим
                       # client_max_body_size — он в образе статикой, переменной не следует
...
## Пользовательский адрес LLM

Адрес своего LLM приходит в запросе и становится `base_url` клиента, то есть по нему
идёт сам сервер. Поэтому адрес проверяется до обращения — разрешены только `http` и
`https`, а имя хоста не должно разрешаться во внутреннюю сеть (loopback, RFC1918,
link-local, CGNAT, site-local IPv6, служебные диапазоны). Редиректы для запросов к LLM
отключены, иначе публичный адрес с ответом `302` уводил бы запрос внутрь в обход
проверки.

`LLM_ALLOW_PRIVATE_URLS=true` снимает проверку сети — для своего развёртывания, где
LLM стоит рядом (`http://ollama:11434/v1`, адрес в локальной сети). Проверка схемы
остаётся. На публичном развёртывании значение должно быть `false`, интерфейс открыт
без авторизации, и любой посетитель иначе может опрашивать внутреннюю сеть от имени
сервиса. Остаточный риск при `false` — DNS rebinding, имя проверяется до соединения,
а резолвится ещё раз при самом запросе.
